...
public class GenericGraphSourceTest {
  ...
  public void getVertex() throws ScriptException {
    ...
    assert(IteratorUtils.count((Iterator) engine.eval("g.V(1).repeat(out().simplePath()).until(hasId(5)).path().limit(1)",bindings)) ==
            IteratorUtils.count(g.traversal().V(1).repeat(out().simplePath()).until(hasId(5)).path().limit(1)));
    System.out.println(IteratorUtils.asList(g.traversal().V(1).repeat(out().simplePath()).until(hasId(3)).path()));
  }
